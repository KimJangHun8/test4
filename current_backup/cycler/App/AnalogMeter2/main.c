...
int main(void)
{
	...
	while(myData->AppControl.signal[APP_SIG_ANALOG_METER2_PROCESS] == P1) {
...
//	int rtn;
	...
	myPs = &(myData->AnalogMeter2);

	Init_SystemMemory();

	memset((char *)&psName[0], 0x00, 16);
	strcpy(psName, "Analog2");
...
/*	rtn = ComPortInitialize(myPs->config.comPort, myPs->config.comBps);
	if(rtn < 0) return -4;
	myPs->config.ttyS_fd = rtn;*/
	myData->AppControl.signal[APP_SIG_ANALOG_METER2_PROCESS] = P1;
...
void Close_Process(void)
{
	...
	myData->AppControl.signal[APP_SIG_ANALOG_METER2_PROCESS] = P3;	
