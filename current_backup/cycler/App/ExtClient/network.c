...
int send_cmd_ch_data(void)
{
	...
	int count;
	#endif
	diff= labs(myData->mData.misc.timer_mSec 
			- myPs->sended_save_data_time);
	if(diff >= myPs->config.send_save_data_interval) {
		myPs->sended_save_data_time 
			= myData->mData.misc.timer_mSec;
		#ifdef __LG_VER1__
		rtn = send_cmd_ch_data_save();
		if(rtn >= 0){
			if(rtn > 0) count = 10;
			else count = 0;
			while(count) {
	...
	if(diff >= myPs->config.send_monitor_data_interval) {
		myPs->sended_monitor_data_time 
			= myData->mData.misc.timer_mSec;
		rtn = send_cmd_ch_data_idle();
		if(rtn >= 0){
		}else{
			return rtn;
		}
...
{
	int cmd_size, body_size, rtn, bd, ch, i, count, msg, day, hour, min, sec, ms, code, tmp, slave_ch;
			...
			if(myData->bData[bd].cData[ch].ChAttribute.opType == P1) {
				...
				if(myData->bData[bd].cData[ch].op.state != C_RUN){
					...
					switch(myData->bData[bd].cData[ch].op.type){
						...
						case STEP_DISCHARGE:
						case STEP_Z:
							sprintf((char *)& cmd.cData.Isens, "%07ld", labs(tmp/1000));
							break;
						default:
							sprintf((char *)& cmd.cData.Isens, "%07d", 0);
							break;
...
/*				if(myData->bData[bd].cData[ch-1].ChAttribute.opType == P1) {
					sprintf((char *)& cmd.cData.Isens, "%07d", 0);
					sprintf((char *)&cmd.cData.capacity, "%07d", 0);
	
					sprintf((char *)&cmd.cData.watt, "%07d", 0);
					sprintf((char *)&cmd.cData.wattHour, "%07d", 0);
					sprintf((char *)&cmd.cData.step_time_day, "%03d", 0);
					sprintf((char *)&cmd.cData.step_time_hour, "%02d", 0);
					sprintf((char *)&cmd.cData.step_time_min, "%02d", 0);
					sprintf((char *)&cmd.cData.step_time_sec, "%02d", 0);
					sprintf((char *)&cmd.cData.step_time_mSec, "%03d", 0);
				}else{*/
					if(myData->bData[bd].cData[ch].op.state != C_RUN){
						sprintf((char *)& cmd.cData.Isens, "%07d", 0);
					}else{
						switch(myData->bData[bd].cData[ch].op.type){
							...
							case STEP_Z:
								sprintf((char *)& cmd.cData.Isens, "%07d", 
									(int)labs(myData->bData[bd].cData[ch].op.Isens/1000));
								break;
							default:
								sprintf((char *)& cmd.cData.Isens, "%07d", 0);
								break;
...
//			}
			...
			sprintf((char *)&cmd.cData.z, "%07d",
				(int)myData->bData[bd].cData[ch].op.z);//uOhm
			...
			sprintf((char *)&cmd.cData.temp, "%07d",
				(int)myData->bData[bd].cData[ch].op.temp);
	...
	for(i=0; i < myData->mData.config.installedCh; i++) {
		...
		if(myData->save_msg[msg].write_idx[bd][ch]
			...
			switch(myData->save_msg[msg].val[idx][bd][ch].chData.type){
				...
				case STEP_DISCHARGE:
				case STEP_Z:
					sprintf((char *)& cmd.cData.Isens, "%07ld", labs(tmp)); 
					break;
				default:
					sprintf((char *)& cmd.cData.Isens, "%07d", 0);
					break;
			...
			tmp = myData->save_msg[msg].val[idx][bd][ch].chData.z;//uOhm
			...
			tmp = myData->save_msg[msg].val[idx][bd][ch].chData.temp;
			sprintf((char *)&cmd.cData.temp, "%07d", tmp);
...
*/
	...
	if(myData->bData[bd].cData[ch].ChAttribute.opType == P1){
		myData->bData[bd].cData[ch+1].signal[C_SIG_STOP] = P1;
	}
...
*/
	...
	if(myData->bData[bd].cData[ch].ChAttribute.opType == P1){
		myData->bData[bd].cData[ch+1].signal[C_SIG_PAUSE] = P1;
	}
...
//	Convert_testCond(bd, ch, parallel, cmd);
	...
	for( i = 0; i < 4; i++){
		...
		if(rtn == 1){
			...
			myData->mData.testCond[bd][ch].step[i].z_t2 = tmp;
