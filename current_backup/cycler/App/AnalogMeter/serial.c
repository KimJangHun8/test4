...
int rcv_cmd_answer(void)
{
	...
	if(myPs->signal[ANALOG_METER_SIG_MEASURE] <= 19){
		...
		if(myPs->signal[ANALOG_METER_SIG_MEASURE] >= 110 &&
			myPs->signal[ANALOG_METER_SIG_MEASURE] <= 111){
			flag = 2; //ambient temp
		}else if(myPs->signal[ANALOG_METER_SIG_MEASURE] >= 210 &&
			myPs->signal[ANALOG_METER_SIG_MEASURE] <= 211){
			flag = 3; //gas
		}else{ //for 10 Module
			flag = 1; //temp
		}
...
//	printf("\n");
	...
	if(myPs->config.checkSumFlag){
		...
		if(flag == 3){	//gasVoltage
			data_size = (7 * myPs->config.chPerModule2) + 5;		//M-7002
		}else{	//temp, ambientTemp
...
/*				index = (int)myPs->signal[ANALOG_METER_SIG_MEASURE] % 10;
	...
	if(myPs->signal[ANALOG_METER_SIG_MEASURE] <= 19){
		...
		if(myPs->signal[ANALOG_METER_SIG_MEASURE] >= 110 &&
			myPs->signal[ANALOG_METER_SIG_MEASURE] <= 111){
			temp_bd = (int)(myPs->signal[ANALOG_METER_SIG_MEASURE] % 10);
			flag = 2; //ambient temp
		}else if(myPs->signal[ANALOG_METER_SIG_MEASURE] >= 210 &&
			myPs->signal[ANALOG_METER_SIG_MEASURE] <= 211){
			temp_bd = (int)(myPs->signal[ANALOG_METER_SIG_MEASURE] % 10);
...
//	temp_bd = (int)(myPs->signal[ANALOG_METER_SIG_MEASURE] % 10) - 1;
	...
	if(flag == 1 || flag == 2){
		...
		for(i=0;i < myPs->config.chPerModule2 ;i++){ //M-7002
			if((answer.data[(i*7)+4] != '+') && ( answer.data[(i*7)+4] != '-')) {
				userlog(DEBUG_LOG, psName, "answer gas data error1 \n");
				next_module_request();
				return 0;
			}
		}
	...
	for(i = 1 ; i<100; i++) {
		if(answer.data[i] == '+' || answer.data[i] == '-') {
			...
			if(flag == 1 || flag == 2){
				if(temp_ch == (myPs->config.chPerModule -1)) break;
				else temp_ch++;
			}else if(flag == 3){
				//M7002 - 4Ch
				//M7003 - 8Ch
				if(temp_ch == (myPs->config.chPerModule2 - 1))	//0Base
					break;
				else
					temp_ch++;
			}
	...
	if(flag == 1){
	...
	}else if(flag == 2){	//ambient temp
		for(temp_ch = 0 ; temp_ch < myPs->config.chPerModule ; temp_ch++){
			chPerModule = myPs->config.chPerModule;
			myData->mData.misc.ambientTemp_org[chPerModule * temp_bd + temp_ch]
				= (long)(temp[temp_ch]);
			myData->mData.misc.ambientTemp[chPerModule * temp_bd + temp_ch]	
				= (long)(temp[temp_ch] * myPs->config.ambient_gain[temp_bd][temp_ch])
				+ myPs->config.ambient_offset[temp_bd][temp_ch];
		}
		for(i = 0 ; i < myData->mData.config.installedCh; i++){
			if(myData->ChamberChNo[i].number1 != 0 &&
				myData->ChamberChNo[i].number1 <= 
					myPs->config.chPerModule * myPs->config.ambientModuleCount){
				chamberNo = myData->ChamberChNo[i].number1;
		...
		//M-7002 : max ch 4EA(1 module)
		//M-7003 : max ch 8EA(1 module)
		for(temp_ch = 0 ; temp_ch < myPs->config.chPerModule2 ; temp_ch++){ 
			chPerModule2 = myPs->config.chPerModule2;
			myData->mData.misc.gasVoltage_org[chPerModule2 * temp_bd + temp_ch]
				= (long)(temp[temp_ch]);
			myData->mData.misc.gasVoltage[chPerModule2 * temp_bd + temp_ch] 
				= (long)(temp[temp_ch] * myPs->config.gas_gain[temp_bd][temp_ch])
				+ myPs->config.gas_offset[temp_bd][temp_ch];
		}
		for(i = 0 ; i < myData->mData.config.installedCh; i++){
			if(myData->ChamberChNo[i].number1 != 0 && 
				myData->ChamberChNo[i].number1 <= 
					myPs->config.chPerModule2 * myPs->config.gasModuleCount ){
				chamberNo = myData->ChamberChNo[i].number1;
				bd1 = myData->ChamberChNo[i].bd;
				ch1 = myData->ChamberChNo[i].ch;
				myData->bData[bd1].cData[ch1].misc.gasVoltage
					= myData->mData.misc.gasVoltage[chamberNo-1] * 1000;
			}
		}
	}
...
void next_module_request() {
	if(myPs->signal[ANALOG_METER_SIG_MEASURE] == P11) {
		...
		//	myPs->signal[ANALOG_METER_SIG_MEASURE] = P1;
			if(myPs->config.ambientModuleCount > 0) {
				myPs->signal[ANALOG_METER_SIG_MEASURE] = P100;
			}else{
				if(myPs->config.gasModuleCount > 0) {
					myPs->signal[ANALOG_METER_SIG_MEASURE] = P200;
				}else{
					if(myPs->config.countMeter > 0){
						myPs->signal[ANALOG_METER_SIG_MEASURE] = P1;
		...
		myPs->signal[ANALOG_METER_SIG_MEASURE_ERROR] = 0;
	}else if(myPs->signal[ANALOG_METER_SIG_MEASURE] == P110) {
		if(myPs->config.ambientModuleCount > 1) {
			myPs->signal[ANALOG_METER_SIG_MEASURE] = P101;
		}else{
			if(myPs->config.gasModuleCount > 0) {
				myPs->signal[ANALOG_METER_SIG_MEASURE] = P200;
			}else{
		...
		myPs->signal[ANALOG_METER_SIG_MEASURE_ERROR] = 0;
	}else if(myPs->signal[ANALOG_METER_SIG_MEASURE] == P111) {
		if(myPs->config.gasModuleCount > 0) {
			myPs->signal[ANALOG_METER_SIG_MEASURE] = P200;
		}else{
			if(myPs->config.countMeter > 0){
		...
		myPs->signal[ANALOG_METER_SIG_MEASURE_ERROR] = 0;
	}else if(myPs->signal[ANALOG_METER_SIG_MEASURE] == P210) {
		if(myPs->config.gasModuleCount > 1) {
			myPs->signal[ANALOG_METER_SIG_MEASURE] = P201;
		}else{
			if(myPs->config.countMeter > 0){
				myPs->signal[ANALOG_METER_SIG_MEASURE] = P1;
			}else if(myPs->config.ambientModuleCount > 0){
		...
		myPs->signal[ANALOG_METER_SIG_MEASURE_ERROR] = 0;
	}else if(myPs->signal[ANALOG_METER_SIG_MEASURE] == P211) {
		if(myPs->config.countMeter > 0){
			myPs->signal[ANALOG_METER_SIG_MEASURE] = P1;
		}else if(myPs->config.ambientModuleCount > 0){
			myPs->signal[ANALOG_METER_SIG_MEASURE] = P100;
...
{	//190807 pthw
	int i, j, k;
	unsigned char  bd, ch, AuxTCnt, checkNum, startNum, auxStartCh;
	unsigned short installedAuxT;

	installedAuxT = myData->mData.config.installedTemp;
	auxStartCh = myPs->config.auxStartCh;
			
	AuxTCnt = checkNum = startNum = 0;
	for(i = 0 ; i < myData->mData.config.installedCh; i++){
		...
		for(j = 0; j < installedAuxT; j++){
			if(myData->auxSetData[j].auxType == 0 
				...
				myPs->chData_AuxT[checkNum] = myPs->temp[j + auxStartCh].temp;
				checkNum++;
				myData->bData[bd].cData[ch].misc.chAuxTCnt++;
				myData->bData[bd].cData[ch].misc.chAuxTCheckNum = checkNum;
				continue;
			}
		}

		for(j = checkNum; j < installedAuxT; j++){
			myPs->chData_AuxT[j] = 0;
		}	
		
		AuxTCnt = myData->bData[bd].cData[ch].misc.chAuxTCnt;
		startNum = myData->bData[bd].cData[ch].misc.chAuxTCheckNum - AuxTCnt;

		if(AuxTCnt > MAX_CH_AUX_DATA) AuxTCnt = MAX_CH_AUX_DATA;

		for(k= 0; k < MAX_CH_AUX_DATA; k++){
			if(k < AuxTCnt){
				myData->bData[bd].cData[ch].misc.chAuxTemp[k]
					= myPs->chData_AuxT[k + startNum];
			}else{
...
void send_cmd_request1(int type, int bd)
{
	...
	if(type == 0) {
		...
		} else if(bd == 31){ //ambient temp
			memcpy((char *)&cmd.data[0], "#1F\r", cmd_size);
		} else if(bd == 32){ //ambient temp
			memcpy((char *)&cmd.data[0], "#20\r", cmd_size);
		} else if(bd == 41){ //gas Voltage
			cmd_size = 5;
			if(myPs->config.chPerModule2== 4){
				memcpy((char *)&cmd.data[0], "$294\r", cmd_size);
			}else if(myPs->config.chPerModule2 == 8){
				memcpy((char *)&cmd.data[0], "$298\r", cmd_size);
			}
		} else if(bd == 42){ //gas Voltage
			cmd_size = 5;
			if(myPs->config.chPerModule2 == 4){
				memcpy((char *)&cmd.data[0], "$2A4\r", cmd_size);
			}else if(myPs->config.chPerModule2 == 8){
				memcpy((char *)&cmd.data[0], "$2A8\r", cmd_size);
			}
		} else if(bd == 45){ //gas Voltage
			memcpy((char *)&cmd.data[0], "#**\r", cmd_size);
		}
	} else if(type == 1) {
		cmd_size = 5;
		if(bd == 1) {
