...
int main(void)
{
	...
	while(myData->AppControl.signal[APP_SIG_CALI_METER_PROCESS] == P1) {
		...
		//180312 add for 232 to Lan Converter
...
int	Initialize(void)
{
    ...
    if(myPs->config.Shunt_Sel_Calibrator != 0 ){
...
void Check_Signal(void)
{
	...
	switch(myPs->signal[CALI_METER_SIG_INITIALIZE]) {
		...
		case P24:
			send_cmd_initialize(4);
			//kjgw send_msg(METER_TO_MAIN, MSG_METER_MAIN_INITIALIZE_REPLY, 0, 0);
			myPs->signal[CALI_METER_SIG_INITIALIZE] = P0;
			break;
		case P31:
			send_cmd_initialize(1);
	...
	if(myPs->signal[CALI_METER_SIG_REQUEST_PHASE] == P2) {
		send_msg(METER_TO_MODULE, MSG_METER_MODULE_REQUEST_REPLY,
