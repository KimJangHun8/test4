...
int SerialCommand_Receive(void)
{
	...
	if(myPs->rcvCmd.cmdBuf[0] == '+' || myPs->rcvCmd.cmdBuf[0] == '-') {
		...
		//170207 add for cmdBufSize Init
...
//	long real, real1, temp;
	...
	unsigned char offset_val = 0;		//kjc_200309
	int type = 0;
	...
	if(myData->mData.config.ratioVoltage == 1){ //20190916
		ratioV = 1000000.0;
	} else {
		ratioV = 1000.0;
	}
	if(myData->mData.config.ratioCurrent == 1){ //20190916
		ratioI = 1000000.0;
	} else {
		ratioI = 1000.0;
	}
	...
	offset_val = myPs->offset_val;		//kjc_200309
	type = myPs->caliType;				//kjc_200309

	if(myData->mData.cali_hallCT == P1	//hun_210830
		|| myData->mData.cali_hallCT == P3){
		myPs->shunt_mOhm = myPs->hallCT_ratio;
		printf("ratio	: ");
		printf("%c[1;33m",27);	//33:YELLOW
		printf("%.3f",myPs->shunt_mOhm);
	...
	if(myPs->caliType == CALI_I) {
		...
		if(myData->mData.cali_hallCT == P1		//hun_210830
			|| myData->mData.cali_hallCT == P3){
			diff = (real*myPs->shunt_mOhm / 100000)
...
//		printf("%ld", myData->bData[bd].cData[ch].misc.cmd_i/ratioI);
		...
		//hun_200217_s
		myPs->orgValue = real / 100000;
		printf("Meter_org: ");
		printf("%c[1;31m",27);	//31:RED
		printf("%.5f", (float)myPs->orgValue);
		printf("%c[0m",27);		//default
		if(myData->mData.cali_hallCT == P1		//hun_210830
			|| myData->mData.cali_hallCT == P3){
			printf(" mA\n");
		...
		#if SHUNT_R_RCV == 2
		printf("Set_Offset: ");
		printf("%c[1;32m",27);	//32:GREEN
		printf("%.5f", (float)myData->cali.tmpCond[bd][ch]
						.point[type][rangeI].meter_offset[offset_val]);
		printf("%c[0m",27);		//default
		if(myData->mData.cali_hallCT == P1	//hun_210830
			|| myData->mData.cali_hallCT == P3){
			printf(" mA\n");
		...
		printf("%c[1;32m",27);	//32:GREEN
		printf("%.5f", (float)myPs->orgValue + 
			(float)myData->cali.tmpCond[bd][ch]
				.point[type][rangeI].meter_offset[offset_val]);
		printf("%c[0m",27);		//default
		if(myData->mData.cali_hallCT == P1		//hun_210830
			|| myData->mData.cali_hallCT == P3){
		...
		if(myData->mData.cali_hallCT == P1		//hun_210830
			|| myData->mData.cali_hallCT == P3){
			myPs->value = (real	* myPs->shunt_mOhm);
		}else{
		...
		printf("Diff	: ");
		if(labs(diff)
			> labs((myData->mData.config.maxCurrent[rangeI]/1000)*0.001)) {
		...
		printf("cmd_v	: ");
		printf("%c[1;33m",27);	//33:YELLOW
//		printf("%ld", myData->bData[bd].cData[ch].misc.cmd_v/ratioV);
		printf("%.3f", (float)myData->bData[bd].cData[ch].misc.cmd_v/ratioV);
		printf("%c[0m",27);		//default
		printf(" mV\n");
	...
	if(myData->mData.config.FadBdUse == P1){
		printf("Fad V : ");
		printf("%c[1;31m",27);	//31:RED
		printf("%.3f",myData->cali.orgAD_fad[0]/1000.0);
		printf("%c[0m",27);		//default
		printf(" mV I :");
		printf("%c[1;31m",27);	//31:RED
		printf("%.3f",myData->cali.orgAD_fad[1]/1000.0);
		printf("%c[0m",27);		//default
	   printf(" mA\n\n");
	}
