...
{
	int i, cmd_size=0, cmdBuf_index, tmp;
	...
	if(myPs->rcvCmd.cmdBuf[0] == 0x02) {
		for(i=0; i < myPs->rcvCmd.cmdBufSize; i++) {
			if(i == 3) { //body_size
				tmp = 6 + (int)myPs->rcvCmd.cmdBuf[i];
			}
			if((i+1) == tmp && myPs->rcvCmd.cmdBuf[i] == 0x03) {
...
int rcv_cmd_answer(void)
{
	...
	real = (long)answer.body[1] * 100000;
	real += (long)answer.body[2] * 10000;
	real += (long)answer.body[3] * 1000;
	real += (long)answer.body[5] * 100;
	real += (long)answer.body[6] * 10;
	real += (long)answer.body[7];
	if(answer.body[0] == '-') real *= (-1);

	myPs->value = real;
	userlog(DEBUG_LOG, psName, "RcvCmd real : %d(mV/mA)\n", real);
...
void send_cmd_request(void)
{
	...
	bcc ^= cmd.data[1];
	cmd.data[2] = 0x10; //CMD_READ_MAGNET_I
	bcc ^= cmd.data[2];
