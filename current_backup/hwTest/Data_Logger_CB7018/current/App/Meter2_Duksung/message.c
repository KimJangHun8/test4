...
void rcv_msg(int fromPs)
{
	...
	switch(fromPs) {
		case APP_TO_METER2:
			...
			break;
		case DATASAVE_TO_METER2:
			rtn = msgParsing_DataSave_to_Meter2(msg, ch, val, idx);
			break;
		default:
...
int msgParsing_App_to_Meter2(int msg, int ch, int val, int idx)
{
	...
	switch(msg) {
		case MSG_APP_METER2_REQUEST:
			send_cmd_request();
			break;
		case MSG_APP_METER2_LOG_START:
			myPs->signal[METER_SIG_LOG_START] = PHASE1;
			break;
		case MSG_APP_METER2_LOG_STOP:
...
int msgParsing_DataSave_to_Meter2(int msg, int ch, int val, int idx)
{
	int rtn=0;
	...
	switch(msg) {
		default:
			userlog(DEBUG_LOG, psName,
				"DataSave to Meter2 Msg Unknown : %d %d %d\n", msg, ch, val);
			break;
	}
	return rtn;
}
