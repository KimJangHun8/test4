...
{ //Agilent 34401A
	...
	memset((char *)&header, 0x00, sizeof(S_METER1_CMD_HEADER));
	memcpy((char *)&header, (char *)&myPs->rcvCmd.cmd[0],
		sizeof(S_METER1_CMD_HEADER));
	
	if(myPs->config.CmdRcvLog == PHASE1) {
		if(myPs->config.CommCheckLog == PHASE1) {
			userlog(METER1_LOG, psName, "recvCmd %s:end\n", myPs->rcvCmd.cmd);
		} else {
			userlog(METER1_LOG, psName, "recvCmd %s:end\n", myPs->rcvCmd.cmd);
		}
	}
	...
	if(myPs->config.CmdRcvLog_Hex == PHASE1) {
		if(myPs->config.CommCheckLog == PHASE1) {
			userlog(METER1_LOG, psName, "recvCmd");
			for(i=0; i < myPs->rcvCmd.cmdSize; i++) {
				userlog2(METER1_LOG, psName, " %x", myPs->rcvCmd.cmd[i]);
			}
			userlog2(METER1_LOG, psName, ":end\n");
		} else {
			userlog(METER1_LOG, psName, "recvCmd");
			for(i=0; i < myPs->rcvCmd.cmdSize; i++) {
				userlog2(METER1_LOG, psName, " %x", myPs->rcvCmd.cmd[i]);
...
int CmdHeader_Check(char *rcvHeader)
{
	...
	length = sizeof(S_METER1_CMD_HEADER);
...
int rcv_cmd_answer(void)
{
	...
	S_METER1_RCV_CMD_ANSWER	answer;
	
	memset((char *)&answer, 0x00, sizeof(S_METER1_RCV_CMD_ANSWER));
	memcpy((char *)&answer, (char *)&myPs->rcvCmd.cmd[0],
		sizeof(S_METER1_RCV_CMD_ANSWER));
	
	memset(buf, 0x00, sizeof buf);
	memcpy((char *)&buf[0], (char *)&answer.header.sign1, 15);
	val = strtod(buf, (char **)NULL);
	//kjgd userlog(DEBUG_LOG, psName, "RcvCmd value : %f\n", val);
	...
	myPs->value = real;
	userlog(DEBUG_LOG, psName, "RcvCmd real : %f(mV/mA)\n",
		(float)real/100000.0);
...
{
	int rtn, cmd_size;
	S_METER1_SEND_CMD_INITIALIZE	cmd;
	
	memset((char *)&cmd, 0x00, sizeof(S_METER1_SEND_CMD_INITIALIZE));

	/*make_header((char *)&cmd, REPLY_NO,
		METER1_SEND_CMD_REQUEST, SEQNUM_DFLT, body_size); kjgw*/
...
{
	int rtn, cmd_size;
	S_METER1_SEND_CMD_REQUEST	cmd;
	
	memset((char *)&cmd, 0x00, sizeof(S_METER1_SEND_CMD_REQUEST));
...
void make_header(char *cmd, char reply, int cmd_id, int seqno, int body_size)
{
	...
	memcpy(cmd, (char *)&header, sizeof(S_METER1_CMD_HEADER));
	
/*	userlog(METER1_LOG, psName, "header");
	for(i=0; i < sizeof(S_METER1_CMD_HEADER); i++) {
		userlog2(METER1_LOG, psName, " %x", *(cmd + i));
	}
	userlog2(METER1_LOG, psName, ":end\n");
	userlog(METER1_LOG, psName, "header %s:end\n", cmd); //for debug kjg*/
}
...
int	send_command(char *cmd, int size, int cmd_id)
{
	...
	if(myPs->config.CmdSendLog == PHASE1) {
		if(myPs->config.CommCheckLog == PHASE1) {
			userlog(METER1_LOG, psName, "sendCmd %s:end\n", packet);
		} else {
			userlog(METER1_LOG, psName, "sendCmd %s:end\n", packet);
		}
	}
	...
	if(myPs->config.CmdSendLog_Hex == PHASE1) {
		if(myPs->config.CommCheckLog == PHASE1) {
			userlog(METER1_LOG, psName, "sendCmd");
			for(i=0; i < size; i++) {
				userlog2(METER1_LOG, psName, " %x", *(cmd + i));
			}
			userlog2(METER1_LOG, psName, ":end\n");
